...
function connectToPeerCode(raw){
 var id=String(raw||"").trim().toUpperCase();
 if(!id){toast("Cole o código dela.");return}
 if(!state.peer||!state.peer.open){toast("Seu código ainda está conectando. Aguarde alguns segundos.");return}
 function placeCall(){
  if(!state.localStream){toast("Entre na call primeiro.");return}
  var call=state.peer.call(id,state.localStream);
  if(!call){toast("Não foi possível iniciar a chamada. Confira o código.");return}
  bindCall(call);state.conn=state.peer.connect(id);bindConn(state.conn);toast("Chamando ela…");
 }
 if(!state.inVoice){openCall();getMedia(false).then(function(s){if(!s)return;state.inVoice=true;renderVoice();placeCall()})}else placeCall()
}
function connect(){connectToPeerCode($("connect-code").value)}
...
function applyTheme(theme){
  ...
  localStorage.setItem("entre_nos_theme",theme);
  var s=$("theme-select");if(s)s.value=theme;
}
function toggleFullscreen(){
  if(!document.fullscreenElement){
    document.documentElement.requestFullscreen().then(function(){
      document.body.classList.add("fullscreen-active"); document.documentElement.classList.add("fullscreen-active");
      var b=$("fullscreen-btn");if(b)b.classList.add("active");
    }).catch(function(){toast("O navegador bloqueou a tela cheia.")});
  }else{
...
document.addEventListener("fullscreenchange",function(){
  ...
  document.body.classList.toggle("fullscreen-active",active); document.documentElement.classList.toggle("fullscreen-active",active);
  var b=$("fullscreen-btn");if(b)b.classList.toggle("active",active);
});

function initEvents(){
 $("login-name").value=state.user?state.user.name:"";setupLogin();
 if(state.user)openApp();
 applyTheme(localStorage.getItem("entre_nos_theme")||"dark");
 ...
 $("connect-btn").onclick=connect;$("call-connect-btn").onclick=function(){connectToPeerCode($("call-connect-code").value)};$("call-connect-code").addEventListener("keydown",function(e){if(e.key==="Enter"){e.preventDefault();$("call-connect-btn").click()}});$("copy-code").onclick=$("copy-my-code").onclick=function(){var code=state.peerId||$("my-code").textContent;navigator.clipboard?navigator.clipboard.writeText(code).then(function(){toast("Código copiado.")}):toast(code)};
 ...
 $("user-settings-btn").onclick=function(){setView("settings")};
 $("fullscreen-btn").onclick=toggleFullscreen;
 $("settings-fullscreen").onclick=toggleFullscreen;
 $("theme-select").onchange=function(){applyTheme(this.value)};
 applyTheme(localStorage.getItem("entre_nos_theme")||"dark");
$("app-settings-btn").onclick=function(){setView("settings")};$("call-settings").onclick=function(){setView("settings");closeCall()};$("save-account").onclick=function(){var n=$("account-name-input").value.trim();if(n){state.user.name=n;save();openApp();toast("Conta atualizada.")}};$("test-mic").onclick=function(){toast("Microfone selecionado. Fale para testar o nível.")};
...
/* ===== SCREEN SHARE: SENDER + RECEIVER ===== */
let localScreenStream = null;
let remoteScreenStream = null;

function showRemoteScreen(show=true){
  const box=$("remote-screen-share");
  if(!box) return;
  box.classList.toggle("hidden", !show);
}
function attachRemoteScreen(stream){
  const v=$("remote-screen-video"), box=$("remote-screen-share");
  if(!v || !box) return;
  remoteScreenStream=stream;
  v.srcObject=stream;
  box.classList.add("has-video");
  box.classList.remove("hidden");
  const n=$("remote-screen-name");
  if(n) n.textContent=(state && state.partnerName ? state.partnerName+" está transmitindo a tela" : "Ela está transmitindo a tela");
  v.play().catch(()=>{});
}
function stopRemoteScreen(){
  const v=$("remote-screen-video"), box=$("remote-screen-share");
  if(v) v.srcObject=null;
  if(box) box.classList.add("hidden");
  remoteScreenStream=null;
}
async function startScreenShareFixed(){
  if(!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia){
    toast("Seu navegador não permite compartilhamento de tela.");
    return;
  }
  try{
    ...
    // Support the common peer connection patterns used by this project.
    const pc = window.peerConnection || window.pc || window.connection || window.currentPeerConnection;
    if(pc && pc.getSenders){
    ...
    const btn=$("share-screen-btn") || $("share-screen");
    if(btn) btn.classList.add("active");
    track.onended=()=>stopScreenShareFixed();
    toast("Você começou a transmitir a tela.");
  }catch(e){
    if(e && e.name!=="AbortError") toast("Não foi possível iniciar a transmissão.");
  }
}
async function stopScreenShareFixed(){
  ...
  // If the app has a camera stream, restore its video track.
  const cam=window.localStream || window.localMediaStream || window.cameraStream;
  const pc=window.peerConnection || window.pc || window.connection || window.currentPeerConnection;
  if(pc && pc.getSenders && cam){
...
function handleIncomingTrack(event){
  ...
  if(hasVideo) attachRemoteScreen(stream);
}
function bindRemotePeerConnection(pc){
  if(!pc || pc.__screenShareBound) return;
  pc.__screenShareBound=true;
  pc.addEventListener("track", handleIncomingTrack);
  pc.addEventListener("connectionstatechange", ()=>{
    if(["failed","closed"].includes(pc.connectionState)) stopRemoteScreen();
  });
}
window.entreNosScreenShare={
  start:startScreenShareFixed,
  stop:stopScreenShareFixed,
  attach:attachRemoteScreen,
  bind:bindRemotePeerConnection
};

document.addEventListener("DOMContentLoaded",()=>{
  const b=$("close-remote-screen");
  if(b) b.onclick=()=>showRemoteScreen(false);
  const share=$("share-screen-btn") || $("share-screen");
  if(share){
    share.addEventListener("click",()=>{
      if(localScreenStream) stopScreenShareFixed();
      else startScreenShareFixed();
    });
  }
});
...
setInterval(function(){
  const pc=window.peerConnection || window.pc || window.connection || window.currentPeerConnection;
  if(pc && window.entreNosScreenShare) window.entreNosScreenShare.bind(pc);
},1000);
